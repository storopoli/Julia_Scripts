...
# ╔═╡ 4b411b30-e6d6-11eb-0723-6b0edb778182
begin
	using Chain
	using DataFrames
	using Languages
...
# ╔═╡ 7f69f0c8-8f4a-4969-a0d1-41cefc2a3f69
begin
	using InteractiveUtils
	with_terminal() do
		versioninfo()
...
# ╔═╡ f1c91d1a-b2b0-4310-b61b-7ead95c004c9
TableOfContents(aside=true)

# ╔═╡ 762b9f5e-1fb9-4582-9cc2-a73cc960c73a
md"""
# Análise das Respostas dos Alunos
"""

# ╔═╡ c92a8b63-ce8b-462b-98cd-f7b24935ff50
df = @chain begin
	download("https://github.com/storopoli/Julia_Scripts/blob/master/Pluto/data/responses.xlsx?raw=true")
	XLSX.readtable(1)
	DataFrame(_...)
end

# ╔═╡ 154df329-08b7-4549-9e67-7f31e41ddccc
md"""
São 84 alunos que responderam sobre o uso do Laboratório Virtual durante uma aula segunda-feira 03 de Maio de 2021 à noite.
"""

# ╔═╡ e691d233-c23d-4d6b-9a80-400cc9f3fa68
md"""
## Avaliação do Laboratório Virtual

Algumas coisas são interessantes. Veja a distribuição das seguintes variáveis:

1. **Nota de Inovação** de 1 a 5
2. **Nota de Funcionalidade** de 1 a 5
3. **Nota de Geral** de 1 a 5

A Nota de funcionalidade ficou um pouco menor que as demais por conta de algumas questões de aprimoramento da funcionalidade para os alunos. Isto provavelmente se resolverá durante a implementação e testes adicionais do Laboratório Virtual.
"""

# ╔═╡ e9218327-5b25-44d8-958a-d11b2b449ddb
 begin
	rename!(df, ["timestamp", "email", "name", "RA", "cloud_user", "browser", "rating_innovation", "rating_functionality", "rating_overall", "obs"])
	p1 = @df df histogram(:rating_innovation, title="Inovação")
	p2 = @df df histogram(:rating_functionality, title="Funcionalidade")
	p3 = @df df histogram(:rating_overall, title="Geral")
	plot(p1, p2, p3, layout = (1, 3), legend = false)
end

# ╔═╡ e8fe90fd-3c1e-41d5-adf5-95de48f220bc
md"""
## Navegadores Utilizados

Vamos ver os navegadores que os alunos utilizaram.
"""

# ╔═╡ a5efce1a-9a35-4fea-b943-5cb5966944fd
@chain df begin
	groupby(:browser, skipmissing=true)
	combine(nrow => :n)
	sort(:n, rev=true)
	@df _ bar(:browser, :n, label=false, title="Navegadores Utilizados")
end

# ╔═╡ c1efd66b-c143-44e6-86ac-546b522d4c1b
md"""
## Nuvem de Palavras das Observações Qualitativas
"""

# ╔═╡ 51b08117-c0b5-446d-9dec-91c8bd52f3cc
begin
	stop_words = stopwords(Languages.Portuguese())
	words = @chain df begin
		dropmissing(:obs, disallowmissing = true)
		select(:obs)
		combine(:obs => ByRow(split) => :word)
		flatten(:word)
		groupby(:word)
		combine(nrow => :n)
		sort(:n, rev=true)
		filter(:word => ∉(stop_words), _)
		filter(:word => ∉(["Não", "Nenhuma"]), _)
		wordcloud(_[:, :word], _[:, :n], density = 0.8, mask = shape(ellipse, 800, 600, color="white", backgroundcolor=(0,0,0,0)))
		generate!
...
# ╔═╡ b4414a2b-6280-48b6-9abc-b62c2c752866
md"""
# Ambiente
"""

# ╔═╡ 4f153198-a7ab-47ee-95fe-7fee82062c63
with_terminal() do
...
[[Stuffing]]
deps = ["Random"]
git-tree-sha1 = "2df920c87ecee5d6101f11c35b58767de7140b0e"
...
[[WordCloud]]
deps = ["ColorSchemes", "Colors", "ImageMagick", "ImageTransformations", "Luxor", "Printf", "Random", "Statistics", "Stuffing"]
git-tree-sha1 = "7801946b06c249adc6fdd30212613099129749ea"
